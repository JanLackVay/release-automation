...
def get_console_logs(credentials: Dict[str, str], last_successful_build: int, build_info_url: str) -> str:
    console_log = requests.get(f"{build_info_url}/{last_successful_build}/consoleFull", auth=(credentials["username"],credentials["credentials"]))
...
def get_last_build_information(condition:str, credentials: Dict[str, str], build_info_url: str) -> int:
    console_log = requests.get(f"{build_info_url}/last{condition}Build/buildNumber", auth=(credentials["username"],credentials["credentials"]))
    if not console_log.ok:
        print(console_log.reason)
...
def get_session_information(build_number: int, credentials: Dict[str, str], build_info_url: str) -> bool:
    console_log = requests.get(f"{build_info_url}/{build_number}/api/json", auth=(credentials["username"],credentials["credentials"]))
    ...
    return {"result": result, "branch": branch, "session_id": session_id, "time": time}
